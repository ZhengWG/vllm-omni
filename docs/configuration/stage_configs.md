...
| `CudaIPCConnector` | **Experimental.** Same-host, cross-GPU D2D handoff via CUDA IPC. Works with defaults: the only capacity decision is `pool_size_mb` (GPU pool, allocated **outside** `gpu_memory_utilization` — real usage ≈ gmu + pool). Oversize payloads and full rings degrade to the `/dev/shm` CPU fallback (visible in metrics), never crash. Default ring sizing fits the 64MB container `/dev/shm`; only raising `ring_entries`/`ring_body_max` needs `--shm-size`. | `pool_size_mb` (default `4096`), `max_transfer_mb` (`128`, largest single transfer = slot size; slot count is derived), `keep_on_gpu` (`true`), `keep_on_gpu_keys` (list, stable per-key GPU placement for streamed tensors). Expert: `pool_credits` (explicit slot count override), `small_slot_kb` (`256`), `small_slot_count` (`128`), `inline_threshold_bytes` (`16384`), `ring_body_max` (`131072`), `ring_entries` (auto), `tensor_lifetime_sec` (`30.0`), `zero_copy_recv` (`false`). |
