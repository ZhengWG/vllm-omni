...
# ============================================================================
# CUDA runtime bindings (ctypes over libcudart): IPC mem/event handles,
# async D2D memcpy, stream-wait, and the DLPack view producer used by
# zero-copy receive. Kept as a distinct section so transport logic below
# does not interleave with raw ctypes plumbing.
# ============================================================================
...
# ============================================================================
# Control plane: per-edge SPSC keyed mailbox ring.
#
# One pre-allocated ring per directed edge (opened once) replaces the
# per-transfer /dev/shm round-trip. Sender publishes a fixed-stride entry;
# receiver looks it up by key — fenceless, correct on x86 TSO (ARM/POWER
# would need a store fence).
#
# Lock-free SPSC correctness: body-first / seq-LAST publish (seq is the
# release marker); a seq==0 in-progress sentinel written first on (re)claim;
# a seqlock re-read guarding the body; a per-slot consumed byte for bounded
# backpressure; open addressing by key hash. Pure-Python (struct + POSIX
# shm), no CUDA — testable on CPU.
#
# Fixed header region (``RING_HEADER_BYTES``, written once): see RingHeader.
# Per-slot layout (little-endian):
# seq u64@0 | consumed u8@8 | pclass u8@9 | ts u32@10 | keyhash 16B@14
# | blen u32@30 | body@34.
# ============================================================================
...
# Pool defaults: pool_size_mb (capacity) and max_transfer_mb (largest single
# pool put == big-slot size) are the user-facing pair; the slot count is
# derived. The default slot holds a chat-sized prefill handoff, so common
# prompts never take the CPU fallback out of the box.
_DEFAULT_POOL_SIZE_MB = 4096
_DEFAULT_MAX_TRANSFER_MB = 128
...
class CudaIPCConnector(OmniConnectorBase):
    ...
    def _parse_config(self, config: dict[str, Any]) -> None:
        ...
        # Ring defaults fit the 64MB container /dev/shm default across both
        # pipeline edges; the body size covers any descriptor a default-sized
        # pool put can produce (oversize payloads fall back before publishing).
        self._ring_entries_cfg = int(config.get("ring_entries", 0))  # 0 => auto (see _init_sender_resources)
        self._ring_body_max = int(config.get("ring_body_max", 131072))
        self.local_device = self._resolve_local_device(config.get("local_device", "auto"))
        # Pool sizing: pool_size_mb (capacity budget, allocated OUTSIDE
        # gpu_memory_utilization) and max_transfer_mb (largest single pool
        # put == big-slot size) are the user-facing pair; the slot count is
        # derived. pool_credits remains an expert override with the legacy
        # slot = pool/credits semantics.
        self._pool_size = int(config.get("pool_size_mb", _DEFAULT_POOL_SIZE_MB)) * 1024 * 1024
        max_transfer = int(config.get("max_transfer_mb", _DEFAULT_MAX_TRANSFER_MB)) * 1024 * 1024
        credits_cfg = config.get("pool_credits")
        if credits_cfg is not None:
            self._pool_credits = max(1, int(credits_cfg))
        else:
            self._pool_credits = max(1, self._pool_size // max(1, max_transfer))
    ...
    def _init_sender_resources(self) -> None:
        ...
        # Auto entries bound in-flight unconsumed chunks (>= one per running
        # request), independent of how few big pool slots the edge has.
        n_slots = self._ring_entries_cfg or max(192, self._pool_credits * 4)
    ...
    # --- CUDA IPC handles (ctypes bindings from load_cudart above) ---
