...
class MingFlashOmniForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if self.model_stage == "thinker":
            ...
            # Note: image generation is a separate diffusion stage, it does
            # not go through this AR wrapper — see
            # ``vllm_omni/diffusion/models/ming_flash_omni/pipeline_ming.py``
            # and ``model_arch: MingImagePipeline`` in the stage YAML.
            raise ValueError(
                f"Invalid model_stage: {self.model_stage}. Must be 'thinker' "
                "for the AR stage. For image generation use stage_type: diffusion "
                "with model_arch: MingImagePipeline."
            )
