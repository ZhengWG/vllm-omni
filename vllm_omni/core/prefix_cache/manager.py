...
Owns the request-task table, the hit/span registry, per-step snapshots,
and the merge, and drives the three helpers: ``SlotStatusTable`` (slot
occupancy), ``OutputCapturer`` (split one step's outputs by policy) and
``HitRecaller`` (plan a hit read under the lock, execute it outside).
The controller owns the staging pool, copy queues, and writing rows into
the CPU block pool. The state lock covers the tables only — never a
wait-for-copy, a GPU-byte-budget flush, or a memcpy.
...
from vllm_omni.core.prefix_cache.capture import OutputCapturer, StepOutputs
...
from vllm_omni.core.prefix_cache.occupancy import SlotStatusTable
from vllm_omni.core.prefix_cache.recall import HitRecaller, RecallPlan
...
class OmniPrefixCacheManager:
    def __init__(
        ...
        *,
        ...
        self._capturer = OutputCapturer(self._policy)
        ...
        self._slot_status = SlotStatusTable(config.num_blocks * config.block_size)
        ...
        # Read path: plan() under _state_lock, recall() outside it.
        self._recaller = HitRecaller(
            pool=self._pool,
            slot_status=self._slot_status,
            controller=self._controller,
            lock=self._state_lock,
            publish_writes=self._commit_drained_writes,
        ...
        self._prefetch_queue: deque[tuple[Future, RecallPlan]] = deque()
    ...
    def register_policy(self, policy: ModelCachePolicy) -> None:
        self._policy = policy
        self._capturer = OutputCapturer(policy)
    ...
    def save_outputs(
        ...
        *,
        ...
        step_outputs = self._capturer.capture(
            hidden_states,
            mm_outputs,
    ...
    def materialize(self, step_id: int, req_ids: list[str]) -> StageCacheOutputs:
        ...
        try:
            with self._state_lock:
                ...
                hit_sources: dict[tuple[str, str], RecallPlan | Future] = {}
                for req_id in req_ids:
                    ...
                    slots = self._recaller.hit_slots(hit_upto, hit_blocks)
                    ...
                    for key in keys:
                        ...
                        if fut is not None:
                            ...
                            continue
                        hit_sources[(req_id, key)] = self._recaller.plan(slots, key, req_id)
    ...
    def _prefetch_hit_spans(self) -> None:
        ...
        for req_id, (hit_upto, hit_blocks) in self._hit_spans.items():
            ...
            for key in keys:
                try:
                    plan = self._recaller.plan(slots, key, req_id)
    ...
    @torch.inference_mode()
    def _prefetch_hit(self, plan: RecallPlan, n_new: int) -> torch.Tensor:
        ...
        rows = self._recaller.recall(plan)
    ...
    def _publish_saved_step(
        ...
        *,
        ...
        """Takes ``_state_lock``. Open the pool keys this step writes, submit
        its writes, and store the consume-once snapshot. Copies live hits into
        the snapshot, then clears them. Device→host and GPU-byte-budget flush
        stay outside.
        """
        self._commit_drained_writes()
        for spec in step_outputs.keys_to_open:
            self._ensure_cache_key(spec.key, spec.dtype, spec.feat)
    ...
    def _merge_cached_for_req(
        ...
        slice into the reserved tail. Else cat(recall, new).
        """
        ...
        if isinstance(src, Future):
            ...
            return merged
        cached = self._recaller.recall(src)
