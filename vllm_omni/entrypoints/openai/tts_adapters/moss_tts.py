...
class _MossTTSAdapterBase(ARTTSAdapter):
    ...
    async def build(
        ...
        # MOSS samples internally during build(), before serving_speech copies
        # request.seed onto the stage sampling params. Prefer the request field
        # and fall back to the deploy-config default on the sampling params.
