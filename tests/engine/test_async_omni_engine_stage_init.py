...
def test_resolve_stage_configs_preserves_stage_diffusion_attention_backend(monkeypatch):
    """A stage that already has the shorthand backend must not also receive a
    structured config. The two forms are mutually exclusive later in
    parse_attention_config / OmniDiffusionConfig.from_kwargs.
    """
    import vllm_omni.engine.async_omni_engine as engine_mod
    ...
    assert engine_args.diffusion_attention_backend == "FLASH_ATTN"
