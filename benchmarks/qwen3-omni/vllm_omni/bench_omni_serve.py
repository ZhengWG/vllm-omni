...
and writes a JSON file in the same schema so the qwen3-tts plotter can
ingest it.
...
# Ensure ``benchmarks/qwen3-omni`` (the parent of this file's directory) is on
# sys.path so the shared ``_common`` module is importable when this script is
# invoked from anywhere (CLI, pytest, examples).
...
def create_payload(
    ...
    user_content, mm_processor_kwargs = build_user_content_vllm(query_type, prompt)
    payload: dict = {
        "model": model,
        "messages": [
    ...
    try:
        async with session.post(api_url, json=payload) as response:
            ...
            if not stream:
                ...
                # Non-streaming -> TTFT/TTFP collapse to E2E.
...
async def run_benchmark(
    ...
    print_summary(bench, header="Serving Benchmark Result")
...
async def main(args):
    ...
    sampling_params_list = None if args.no_sampling_params else DEFAULT_SAMPLING_PARAMS_LIST
...
def parse_args():
    ...
    parser.add_argument("--config-name", type=str, default="async_chunk")
